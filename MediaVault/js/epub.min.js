...
!function (a, b) {
    ...
    function G(a, b) {
        ...
        for (var e = 0; d._state === Ba && e < a.length; e++)w(c.resolve(a[e]), void 0, function (a) {
            return s(d, a)
        }, function (a) {
            return v(d, a)
        });
    ...
    ua.mixin(va);
    var wa = void 0;
    wa = Array.isArray ? Array.isArray : function (a) {
        return "[object Array]" === Object.prototype.toString.call(a)
    };
    var xa = wa, ya = Date.now || function () {
        ...
        }, Aa = [], Ba = void 0, Ca = 1, Da = 2, Ea = new y, Fa = new y;
    E.prototype._validateInput = function (a) {
        ...
        if (d === k) {
            var e = n(a);
            if (e === C && a._state !== Ba) a._onError = null, this._settledAt(a._state, b, a._result); else if ("function" != typeof e) this._remaining--, this._result[b] = this._makeResult(Ca, b, a); else if (c === K) {
                var f = new c(m);
    ...
    W.prototype = za(E.prototype), W.prototype._superConstructor = E, W.prototype._makeResult = D, W.prototype._validationError = function () {
        ...
        for (var e in c)b._state === Ba && Object.prototype.hasOwnProperty.call(c, e) && d.push({
            position: e,
            entry: c[e]
        });
        var f = d.length;
    ...
    var Ka = 0, La = void 0, Ma = "undefined" != typeof window ? window : void 0, Na = Ma || {},
...
var EPUBJS = EPUBJS || {};
EPUBJS.VERSION = "0.2.19", EPUBJS.plugins = EPUBJS.plugins || {}, EPUBJS.filePath = EPUBJS.filePath || "/epubjs/", EPUBJS.Render = {}, function (a) {
    var b = (a.ePub, a.ePub = function () {
        var a, b;
        return void 0 !== arguments[0] && ("string" == typeof arguments[0] || arguments[0] instanceof ArrayBuffer) && (a = arguments[0], arguments[1] && "object" == typeof arguments[1] ? (b = arguments[1], b.bookPath = a) : b = {bookPath: a}), !arguments[0] || "object" != typeof arguments[0] || arguments[0] instanceof ArrayBuffer || (b = arguments[0]), new EPUBJS.Book(b)
    });
    ...
    return this.settings.bookPath = a, this.settings.contained || this.isContained(a) ? (this.settings.contained = this.contained = !0, this.bookUrl = "", c = this.unarchive(a).then(function () {
    ...
    var d, e, f = this.element.getBoundingClientRect(), g = b || this.settings.width || f.width,
    ...
    var d = [], e = new EPUBJS.Renderer(this.settings.render_method, !1), f = this.createHiddenRender(e, a, b),
        g = new RSVP.defer, h = -1, i = this.spine.length, j = 0, k = function (a) {
            var b, g = h + 1, l = a || new RSVP.defer;
            if (g >= i) l.resolve(); else {
                if (c && c.cancelled)return e.remove(), this.element.removeChild(f), void l.reject(new Error("User cancelled"));
                h = g, b = new EPUBJS.Chapter(this.spine[h], this.store), e.displayChapter(b, this.globalLayoutProperties).then(function (a) {
    ...
    var b;
    return b = "string" == typeof a ? JSON.parse(a) : a, b && b.length && (this.pageList = b, this.pagination.process(this.pageList), this.ready.pageList.resolve(this.pageList)), this.pageList
    ...
    d.path != e.path ? (console.warn("Miss Match", d.path, this.currentChapter.absolute), b = this.spineIndexByURL[d.filename], c = new EPUBJS.Chapter(this.spine[b], this.store), this.currentChapter = c, this.renderer.currentChapter = c, this.renderer.afterLoad(this.renderer.render.docEl), this.renderer.beforeDisplay(function () {
    ...
    var b, c = EPUBJS.core.uri(a), d = c.protocol, e = "/" == c.path[0], f = window.location,
        g = f.origin || f.protocol + "//" + f.host, h = document.getElementsByTagName("base");
    ...
    var b = this;
    if (EPUBJS.core.isElement(a)) this.element = a; else {
    ...
    var b, c = this,
        ...
        e = !1, f = this.generateBookKey(a), g = localStorage.getItem(f), h = d.length;
    if (this.settings.clearSaved && (e = !0), !e && "undefined" != g && null !== g)for (c.contents = JSON.parse(g), b = 0; b < h; b++) {
        var i = d[b];
        if (!c.contents[i]) {
            e = !0;
            break
    ...
    return this.isRendered ? this._rendering || this.renderer._moving ? (this._displayQ.enqueue("displayChapter", [a, b, i]), i.promise) : (EPUBJS.core.isNumber(a) ? f = a : (e = new EPUBJS.EpubCFI(a), f = e.spinePos), (f < 0 || f >= this.spine.length) && (console.warn("Not A Valid Location"), f = 0, b = !1, e = !1), g = new EPUBJS.Chapter(this.spine[f], this.store), this._rendering = !0, this._needsAssetReplacement() && g.registerHook("beforeChapterRender", [EPUBJS.replace.head, EPUBJS.replace.resources, EPUBJS.replace.svg], !0), h.currentChapter = g, d = h.renderer.displayChapter(g, this.globalLayoutProperties), e ? h.renderer.gotoCfi(e) : b && h.renderer.lastPage(), d.then(function (a) {
    ...
    return this.isRendered ? this.renderer.nextPage() ? (a.resolve(!0), a.promise) : this.nextChapter(a) : (this._q.enqueue("nextPage", [a]), a.promise)
}, EPUBJS.Book.prototype.prevPage = function (a) {
    var a = a || new RSVP.defer;
    return this.isRendered ? this.renderer.prevPage() ? (a.resolve(!0), a.promise) : this.prevChapter(a) : (this._q.enqueue("prevPage", [a]), a.promise)
    ...
    return this.isRendered ? this._moving || this._rendering ? (console.warn("Renderer is moving"), this._gotoQ.enqueue("gotoCfi", [a, h]), !1) : (c = new EPUBJS.EpubCFI(a), (d = c.spinePos) != -1 && (e = this.spine[d], f = h.promise, this._moving = !0, this.currentChapter && this.spinePos === d ? (this.renderer.gotoCfi(c), this._moving = !1, h.resolve(this.renderer.currentLocationCfi)) : (e && d != -1 || (d = 0, e = this.spine[d]), g = this.displayChapter(a), g.then(function (a) {
    ...
    this.contained || this.settings.contained || (this.online && this.opened.then(this.toStorage.bind(this)), this.store && this.settings.fromStorage && a === !1 ? (this.settings.fromStorage = !1, this.store.off("offline"), this.store = !1) : this.settings.fromStorage || (this.store = new EPUBJS.Storage(this.settings.credentials), this.store.on("offline", function (a) {
    ...
    return this.load().then(function (a) {
        ...
        return c.setAttribute("href", this.absolute), b.insertBefore(c, b.firstChild), this.contents = a, new RSVP.Promise(function (b, c) {
    ...
    var b = this, c = [], d = a.toLowerCase(), e = function (a) {
        for (var e, f, g, h = a.textContent.toLowerCase(), i = b.document.createRange(), j = -1, k = 150; f != -1;)f = h.indexOf(d, j + 1), f != -1 && (i = b.document.createRange(), i.setStart(a, f), i.setEnd(a, f + d.length), e = b.cfiFromRange(i), a.textContent.length < k ? g = a.textContent : (g = a.textContent.substring(f - k / 2, f + k / 2), g = "..." + g + "..."), c.push({
            cfi: e,
            excerpt: g
        })), j = f
    };
    ...
    var e, f = {}, g = this.store, h = this.caches[a], i = EPUBJS.core.uri(this.absolute), j = i.base, k = b,
        l = function (a, b, c) {
            f[b] = a
        }, m = function (a) {
    ...
    g && (h || (h = {}), e = EPUBJS.core.clone(h), this.replace(a, function (b, d) {
        var h = b.getAttribute(k), i = EPUBJS.core.resolveUrl(j, h), l = function (c) {
            var e;
            b.onload = function () {
...
var EPUBJS = EPUBJS || {};
EPUBJS.core = {};
var ELEMENT_NODE = 1, TEXT_NODE = 3, COMMENT_NODE = 8, DOCUMENT_NODE = 9;
EPUBJS.core.getEl = function (a) {
    return document.getElementById(a)
}, EPUBJS.core.getEls = function (a) {
    return document.getElementsByClassName(a)
}, EPUBJS.core.request = function (a, b, c) {
    var d, e = window.URL, f = e ? "blob" : "arraybuffer", g = new RSVP.defer, h = new XMLHttpRequest,
        i = XMLHttpRequest.prototype, j = function () {
            var a;
    ...
    return {
        enqueue: d, dequeue: e, flush: function () {
            for (; b.length;)e()
        }, clear: function () {
            b = []
        }, length: function () {
    ...
    }), j = parseInt(i), isNaN(j) || (j % 2 == 0 ? k.steps.push(l(i)) : k.steps.push({
        type: "text",
        index: (j - 1) / 2
    ...
    var c, d, e = EPUBJS.core.uri(a), f = e.path, g = e.fragment, h = b.spineIndexByURL[f], i = new RSVP.defer,
    ...
    return a.prototype.getHooks = function () {
        ...
        e = this.hooks[a], f = e.length, 0 === f && b && b(), e.forEach(function (a) {
            a(d, c)
        })
    }, {
        register: function (a) {
    ...
    var e = EPUBJS.core.prefixed("columnAxis"), f = EPUBJS.core.prefixed("columnGap"),
        g = EPUBJS.core.prefixed("columnWidth"), h = EPUBJS.core.prefixed("columnFill"), i = Math.floor(b),
        j = Math.floor(i / 8), k = d >= 0 ? d : j % 2 == 0 ? j : j - 1;
    ...
    var e = EPUBJS.core.prefixed("columnAxis"), f = EPUBJS.core.prefixed("columnGap"),
        g = EPUBJS.core.prefixed("columnWidth"), h = EPUBJS.core.prefixed("columnFill"), i = 2, j = Math.floor(b),
        k = j % 2 == 0 ? j : j - 1, l = Math.floor(k / 8), m = d >= 0 ? d : l % 2 == 0 ? l : l - 1,
        n = Math.floor((k - m) / i);
    ...
    var e, f, g, h, i = EPUBJS.core.prefixed("columnWidth"), j = EPUBJS.core.prefixed("transform"),
        k = EPUBJS.core.prefixed("transformOrigin"), l = a.querySelector("[name=viewport]");
    ...
    return a.style.position = "absolute", a.style.top = "50%", a.style.left = "50%", a.style[j] = "scale(" + o + ") translate(-50%, -50%)", a.style[k] = "0px 0px 0px", a.style.width = g + "px" || "auto", a.style.height = h + "px" || "auto", a.style[i] = "auto", a.style.overflow = "auto", this.colWidth = g, this.gap = 0, {
        pageWidth: g,
        pageHeight: h
    }
    ...
    var b = new RSVP.defer, c = -1, d = this.spine.length, e = function (a) {
        var b, f = c + 1, g = a || new RSVP.defer;
        return f >= d ? g.resolve() : (c = f, b = new EPUBJS.Chapter(this.spine[c], this.store, this.credentials), this.process(b).then(function () {
            setTimeout(function () {
                e(g)
            }, 1)
        })), g.promise
    }.bind(this);
    return "number" == typeof a && (this.break = a), e().then(function () {
    ...
    return a.load().then(function (b) {
        ...
        this.sprint(g, function (b) {
            ...
            for (0 === h && (c = f.createRange(), c.setStart(b, 0)), g = this.break - h, g > i && (h += i, j = i); j < i;)h = this.break, j += this.break, j >= i ? h = i - (j - this.break) : (c.setEnd(b, j), e = a.cfiFromRange(c), this._locations.push(e), h = 0, j += 1, c = f.createRange(), c.setStart(b, j));
            d = b
    ...
    var b;
    if ("string" == typeof a) this._currentCfi = a; else {
    ...
    return c != -1 ? b = this.pages[c] : (c = EPUBJS.core.locationOf(a, this.locations, this.epubcfi.compare), void 0 !== (b = c - 1 >= 0 ? this.pages[c - 1] : this.pages[0]) || (b = -1)), b
    ...
    return b && (this.baseUrl = b), a ? (c = a.querySelector("metadata")) ? (d = a.querySelector("manifest")) ? (e = a.querySelector("spine")) ? (f = n.manifest(d), g = n.findNavPath(d), h = n.findTocPath(d, e), i = n.findCoverPath(a), j = Array.prototype.indexOf.call(e.parentNode.childNodes, e), k = n.spine(e, f), l = {}, k.forEach(function (a) {
        l[a.href] = a.index
    ...
    if ("2.0" === a.querySelector("package").getAttribute("version")) {
        ...
        if (b) {
            var c = b.getAttribute("content"), d = a.querySelector("item[id='" + c + "']");
    ...
    return Array.prototype.slice.call(d).forEach(function (a) {
        var d = a.getAttribute("id"), e = a.getAttribute("href") || "", f = a.getAttribute("media-type") || "",
            g = a.getAttribute("properties") || "";
        c[d] = {href: e, url: b + e, type: f, properties: g}
    }), c
},EPUBJS.Parser.prototype.spine = function (a, b) {
    var c = a.getElementsByTagName("itemref"), d = Array.prototype.slice.call(c), e = EPUBJS.core.indexOfElementNode(a),
        f = new EPUBJS.EpubCFI;
    return d.map(function (a, c) {
        var d = a.getAttribute("idref"), g = f.generateChapterComponent(e, c, d),
            h = a.getAttribute("properties") || "", i = h.length ? h.split(" ") : [], j = b[d].properties,
            k = j.length ? j.split(" ") : [];
        return {
            id: d,
    ...
    return Array.prototype.forEach.call(e, function (a) {
        if ("li" === a.tagName) {
            var d = a.querySelector("a, span"), e = d.getAttribute("href") || "", g = d.textContent || "",
                h = e.split("#"), i = h[0], j = b[i], k = c[j], l = k ? k.cfi : "", m = this.navItems(a, b, c);
            f.push({href: e, label: g, spinePos: j, subitems: m, cfi: l})
        }
    }.bind(this)), f
},EPUBJS.Parser.prototype.toc = function (a, b, c) {
    var d, e, f, g = a.querySelectorAll("navMap navPoint"), h = g.length, i = {}, j = [];
    if (!g || 0 === h)return j;
    ...
    var d, e = a.getAttribute("id") || !1, f = a.querySelector("content"), g = f.getAttribute("src"),
        h = a.querySelector("navLabel"), i = h.textContent ? h.textContent : "", j = g.split("#"), k = j[0], l = b[k],
        m = c[l], n = [], o = a.parentNode, p = m ? m.cfi : "";
    return o && "navPoint" === o.nodeName && (d = o.getAttribute("id")), e || (l ? (m = c[l], e = m.id, p = m.cfi) : (e = "epubjs-autogen-toc-id-" + EPUBJS.core.uuid(), a.setAttribute("id", e))), {
        id: e,
        href: g,
        label: i,
        spinePos: l,
    ...
    var d, e, f = this.querySelectorByType(a, "nav", "page-list"), g = f ? f.querySelectorAll("ol li") : [],
        h = g.length, i = [];
    if (!g || 0 === h)return i;
    for (d = 0; d < h; ++d)e = this.pageListItem(g[d], b, c), i.push(e);
    return i
},EPUBJS.Parser.prototype.pageListItem = function (a, b, c) {
    var d, e, f, g = (a.getAttribute("id"), a.querySelector("a")), h = g.getAttribute("href") || "",
        i = g.textContent || "", j = parseInt(i), k = h.indexOf("epubcfi");
    return k != -1 ? (d = h.split("#"), e = d[0], f = d.length > 1 && d[1], {
        cfi: f,
        href: h,
        packageUrl: e,
        page: j
    ...
    return this.window && this.unload(), this.iframe && this.element.removeChild(this.iframe), this.iframe = this.addIframe(), this.element.appendChild(this.iframe), this.iframe.onload = function (a) {
    ...
    var b, c;
    return !!(a && "function" == typeof a.getBoundingClientRect && (b = a.getBoundingClientRect(), c = b.left, 0 !== b.width && 0 !== b.height && c >= 0 && c < this.pageWidth))
    ...
    if (this._moving) {
        ...
        return c.reject({message: "Rendering In Progress", stack: (new Error).stack}), c.promise
    }
    ...
    var c = new RSVP.defer;
    return this.layoutMethod = this.determineLayout(this.layoutSettings), this.layout = new EPUBJS.Layout[this.layoutMethod], this.visible(!1), this.render.load(a, b).then(function (a) {
    ...
    var b = this.layout.calculatePages(), c = this.currentChapter, d = this._q.length();
    ...
    var d;
    return !!(a && "function" == typeof a.getBoundingClientRect && (d = a.getBoundingClientRect(), 0 !== d.width && 0 !== d.height && d.left >= b && b <= d.left + d.width))
    ...
    var a, b, c, d, e, f, g, h, i = this, j = [], k = this.render.getBaseElement(), l = 1,
        m = this.layout.colWidth + this.layout.gap, n = this.formated.pageWidth * (this.chapterPos - 1), o = m * l - n,
        p = 0, q = function (b) {
            var c, e, f;
            ...
            return i.splitTextNodeIntoWordsRanges(e).forEach(function (e) {
                ...
                !g || 0 === g.width && 0 === g.height || (g.left + g.width < o ? j[l - 1] || (e.collapse(!0), c = i.currentChapter.cfiFromRange(e), f = j.push({
                        start: c,
                        end: null
                    })) : (!a && d && (b = i.splitTextNodeIntoWordsRanges(d), a = b[b.length - 1]), a && j.length && (a.collapse(!1), c = i.currentChapter.cfiFromRange(a), j[j.length - 1] && (j[j.length - 1].end = c)), e.collapse(!0), c = i.currentChapter.cfiFromRange(e), f = j.push({
                    start: c,
                    end: null
                }), l += 1, o = m * l - n, p = o), a = e)
            }), f
        }, s = this.render.getDocumentElement(), t = s.dir;
    return "rtl" == t && (s.dir = "ltr", "pre-paginated" !== this.layoutSettings.layout && (s.style.position = "static")), this.textSprint(k, q), "rtl" == t && (s.dir = t, "pre-paginated" !== this.layoutSettings.layout && (s.style.left = "auto", s.style.right = "0")), !a && d && (b = i.splitTextNodeIntoWordsRanges(d), a = b[b.length - 1]), a && (a.collapse(!1), c = i.currentChapter.cfiFromRange(a), j[j.length - 1].end = c), j.length || (e = this.doc.createRange(), e.selectNodeContents(k), e.collapse(!0), g = i.currentChapter.cfiFromRange(e), f = this.doc.createRange(), f.selectNodeContents(k), f.collapse(!1), h = i.currentChapter.cfiFromRange(f), j.push({
        start: g,
        end: h
    ...
    return c = !0, void 0 === document.caretPositionFromPoint || c ? void 0 === document.caretRangeFromPoint || c ? (this.visibileEl = this.findElementAfter(a, b), e.setStart(this.visibileEl, 1)) : e = this.doc.caretRangeFromPoint(a, b) : (d = this.doc.caretPositionFromPoint(a, b), e.setStart(d.offsetNode, d.offset)), e
    ...
    var a, b;
    return this.pageMap ? (b = this.pageMap.length, a = this.spreads ? 2 * this.chapterPos - 1 : this.chapterPos, b - a) : (console.warn("page map not loaded"), !1)
},EPUBJS.Renderer.prototype.getVisibleRangeCfi = function () {
    var a, b, c;
    return this.pageMap ? (this.spreads ? (a = 2 * this.chapterPos, b = this.pageMap[a - 2], c = b, this.pageMap.length > 1 && this.pageMap.length > a - 1 && (c = this.pageMap[a - 1])) : (a = this.chapterPos, b = this.pageMap[a - 1], c = b), b || (console.warn("page range miss:", a, this.pageMap), b = this.pageMap[this.pageMap.length - 1], c = b), {
        start: b.start,
        end: c.end
    ...
    if (EPUBJS.core.isString(a) && (a = this.epubcfi.parse(a)), void 0 === document.evaluate) (c = this.epubcfi.addMarker(a, this.doc)) && (b = this.render.getPageNumberByElement(c), this.epubcfi.removeMarker(c, this.doc), this.page(b)); else if (d = this.epubcfi.generateRangeFromCfi(a, this.doc)) {
...
EPUBJS.replace = {}, EPUBJS.replace.hrefs = function (a, b) {
    var c = this, d = function (a, d) {
        ...
        k != -1 ? a.setAttribute("target", "_blank") : (g = b.render.docEl.querySelector("base"), i = g.getAttribute("href"), h = EPUBJS.core.uri(i), e = h.directory, f = e ? "file" === h.protocol ? EPUBJS.core.resolveUrl(h.base, j) : EPUBJS.core.resolveUrl(e, j) : j, a.onclick = function () {
            return c.goto(f), !1
        }), d()
    };
    ...
    if (a)return a.getText(b).then(function (d) {
        EPUBJS.replace.cssImports(a, b, d).then(function (e) {
            d = e + d, EPUBJS.replace.cssUrls(a, b, d).then(function (a) {
                var b = window.URL || window.webkitURL || window.mozURL, d = new Blob([a], {type: "text/css"}),
    ...
    var c = new RSVP.defer, d = a.length, e = 0, f = function (b) {
        var c, g, h = b || new RSVP.defer;
        return e >= d ? h.resolve() : (c = a[e].url, g = window.encodeURIComponent(c), EPUBJS.core.request(c, "binary").then(function (a) {
    ...
    return Array.isArray(a) || (a = [a]), f().then(function () {
        c.resolve()
    }.bind(this)), c.promise
    ...
    return EPUBJS.core.request(a, "arraybuffer", this.withCredentials).then(function (a) {
        ...
        var c = new RSVP.defer, d = EPUBJS.core.getMimeType(a), e = new Blob([b], {type: d}), f = new FileReader;
        ...
        return this.offline || (this.offline = !0, this.trigger("offline", !0)), d ? (d.then(function (b) {
            var d = EPUBJS.core.getMimeType(a), e = new Blob([b], {type: d}), f = new FileReader;
    ...
    var c, d, e = this, f = new RSVP.defer, g = window.decodeURIComponent(a), h = this.zip.file(g),
        i = window.URL || window.webkitURL || window.mozURL;
    return h ? a in this.urlCache ? (f.resolve(this.urlCache[a]), f.promise) : (d = new Blob([h.asUint8Array()], {type: EPUBJS.core.getMimeType(h.name)}), c = i.createObjectURL(d), f.resolve(c), e.urlCache[a] = c, f.promise) : (f.reject({
    ...
    return f ? (c = f.asText(), d.resolve(c), d.promise) : (d.reject({
        message: "File not found in the epub: " + a,
        stack: (new Error).stack
    }), d.promise)
    ...
    var a = {
        ...
        var b, c, d, e, f = {};
        for (b in a)if (a.hasOwnProperty(b))for (c in a[b])if (a[b].hasOwnProperty(c))if ("string" == typeof(d = a[b][c])) f[d] = b + "/" + c; else for (e = 0; e < d.length; e++)f[d[e]] = b + "/" + c;
        return f
    }();
